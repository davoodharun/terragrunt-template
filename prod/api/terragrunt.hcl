...
inputs = {
    ...
    resources = {
        east = {
            resource_group_name = "prod_east"
            location = "East US"
            # merge app settings template files
            appsettings = jsondecode(templatefile("prod.east.appsettings.tpl", local.settings_keys))
        }
        central = {
            resource_group_name = "prod_central"
            location = "Central US"
            # merge app settings template files
            appsettings = jsondecode(templatefile("prod.central.appsettings.tpl", local.settings_keys))
        }
    }
    tags = {
    }
