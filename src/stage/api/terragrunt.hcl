# include "root" {
#   path = find_in_parent_folders("stage.terragrunt.hcl")
# }
...
inputs = {
    ...
    resources = {
        east = {
            ...
            appsettings = jsondecode(templatefile("stage.east.appsettings.tpl", local.settings_keys))
        }
        central = {
            resource_group_name = "stage_central"
            location = "East US"
            # merge app settings template files
            appsettings = jsondecode(templatefile("stage.central.appsettings.tpl", local.settings_keys))
        }
    }
    tags = {
    }
